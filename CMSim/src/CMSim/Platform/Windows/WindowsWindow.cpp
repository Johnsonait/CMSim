...
//This seams to resolve a linker error but I dont know why
...
namespace CMSim {
	...
	{
		CMS_CORE_ERROR("GLFW Error ({0}): {1}", error, description);
	}
	...
	void WindowsWindow::Init(const WindowProps& props)
	{
		...
		CMS_CORE_INFO("Creating Window {0} ({1}, {2})", props.Title, props.Width, props.Height);
		if (!s_GLFWInitialized)
		{
			int success = glfwInit();
			CMS_CORE_ASSERT(success, "Could not initialize GLFW!");
